const distance = {
    recalculateAll() {
        ...
        beams.forEach(beam => {
            ...
            // Rm rzeczywiste (z wysokością mówcy)
            const Rm_vert = +(Math.sqrt(
                Math.pow(Rm_flat, 2) + Math.pow(Math.abs(Hm - hPerson), 2)
            )).toFixed(2);

            speakers.forEach(speaker => {
                // R2: po podłodze
                const R2_flat = +(utils.dist(beam, speaker) * 0.01).toFixed(2);
                // R2 rzeczywiste (z wysokością mówcy)
                const R2_vert = +(Math.sqrt(
                    Math.pow(R2_flat, 2) + Math.pow(Math.abs(H - hPerson), 2)
                )).toFixed(2);

                // Przeliczaj na podstawie rzeczywistych dystansów!
                const Lp2 = logic.Lp2(Lp1, R2_vert, R1);
                const Gain = logic.Gain(Lp1, Lp2);
                const Lsum = logic.Lsum(R2_vert, H);
                const Delay = logic.Delay(Lsum);

                const gainSymetrix = +(Gain + gainCal).toFixed(2);
                const delaySymetrix = +(Delay - Rm_vert).toFixed(2);

                beam.calc[speaker.id] = {
                    gainSymetrix,
                    delaySymetrix,
                    distM_flat: R2_flat,   // dystans po podłodze wiązka–głośnik
                    distM_vert: R2_vert,   // dystans 3D wiązka–głośnik
                    Rm_flat,
                    Rm_vert,
                    Hm,
                    H,
            ...
            // Możesz też zapisać beam.Rm_flat/Rm_vert jeśli potrzebujesz poza .calc
